...
static const int qtab_726_40[15] = { -122, -16, 68, 139, 198, 250, 298, 339,
...
int g726_decode(g726_state_t *s, char out[], const unsigned char g726_data[],
		int g726_bytes) {
	int i;
	int samples;
	unsigned char code;
	...
	for (samples = i = 0;;) {
		...
		code = (unsigned char) ((s->bs.bitstream
	...
	return ShortsToChars(out, amp, samples);
}
...
static int ShortsToChars(char out[], const short src[], int length) {
	int i;
	for (i = 0; i < length; i++) {
		out[2 * i + 0] = (char) (src[i] & 0xff);
		out[2 * i + 1] = (char) (src[i] >> 8 & 0xff);
	}
	return length * 2;
}
